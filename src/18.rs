fn main()
{
    ...
    println!("Part 2: {}", max_magnitude(raw_input()));
}

fn max_magnitude(input: &'static str) -> u32
{
    let numbers: Vec<Number> = input.trim().lines().map(|line| parse(line.trim())).collect();
    ...
    for i in 0..numbers.len() {
        for j in 0..numbers.len() {
            ...
            let magnitude = magnitude(add_and_reduce(a, b));
            if magnitude > max {
                max = magnitude;
            }
        }
