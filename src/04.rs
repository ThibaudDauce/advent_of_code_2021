...
fn part1()
{
    ...
    for input in inputs {
        for board in &mut boards {
            ...
            // for (index, (digit, checked)) in board.iter().enumerate() {
            //     if *digit == input {
            //         board[index].1 = true;
            //     }
            // }
...
fn parse_input() -> (Vec<u32>, Vec<Vec<(u32, bool)>>)
{
    ...
    for block in blocks {
        ...
        let lines = block
            .lines()
            .map(|line| line.trim().split_whitespace().map(|digit| digit.parse().unwrap()))
            ;

        for line in lines {
            for digit in line {
                board.push((digit, false));
            }
        }
