fn main()
{
    println!("Part 1: {}", number_of_ons(raw_input()));
}

fn number_of_ons(input: &'static str) -> i32
{
    let mut operations = vec![];
    for line in input.trim().lines().map(|line| line.trim()) {
        let (status_string, coordinates) = line.split_once(' ').unwrap();

        let status = match status_string {
            "on" => Status::On,
            "off" => Status::Off,
            _ => panic!("Unknown status string {}", status_string),
        };

        let mut parts = coordinates.split(',');
        let (min_x, max_x) = parse_range("x", parts.next().unwrap());
        let (min_y, max_y) = parse_range("y", parts.next().unwrap());
        let (min_z, max_z) = parse_range("z", parts.next().unwrap());

        operations.push(Cuboid { status, min_x, max_x, min_y, max_y, min_z, max_z });
    }

    let mut zones_on: Vec<Cuboid> = vec![];

    for operation in operations {
        let mut new_zones_on = vec![];

        if operation.min_x < -50 || operation.min_x > 50
            || operation.max_x < -50 || operation.max_x > 50
            || operation.min_y < -50 || operation.min_y > 50
            || operation.max_y < -50 || operation.max_y > 50
            || operation.min_z < -50 || operation.min_z > 50
            || operation.max_z < -50 || operation.max_z > 50
        {
            continue;
        }

        for zone_on in zones_on {
            match intersecting(&operation, &zone_on) {
                Intersect::FullyOutside => new_zones_on.push(zone_on),
                Intersect::FullyInside  => {
                    // Nothing to do
                },
                Intersect::SomePartsOutside(mut cuboid_parts) => {
                    new_zones_on.append(&mut cuboid_parts);
                }
            }
        }

        if operation.status == Status::On {
            new_zones_on.push(operation);
        }

        zones_on = new_zones_on;
    }

    count_zones(&zones_on)
}

fn count_zones(zones: &Vec<Cuboid>) -> i32
{
    let mut sum = 0;
    for zone in zones {
        assert_eq!(Status::On, zone.status);
        sum += ((zone.max_x - zone.min_x).abs() + 1) * ((zone.max_y - zone.min_y).abs() + 1) * ((zone.max_z - zone.min_z).abs() + 1);
    }
...
enum Intersect {
    FullyOutside,
    FullyInside,
    SomePartsOutside(Vec<Cuboid>),
}

fn intersecting(new_cuboid: &Cuboid, existing_cuboid: &Cuboid) -> Intersect
{
    if existing_cuboid.min_x > new_cuboid.max_x || existing_cuboid.max_x < new_cuboid.min_x
        || existing_cuboid.min_y > new_cuboid.max_y || existing_cuboid.max_y < new_cuboid.min_y
        || existing_cuboid.min_z > new_cuboid.max_z || existing_cuboid.max_z < new_cuboid.min_z {
            return Intersect::FullyOutside;
    }

    if existing_cuboid.min_x >= new_cuboid.min_x && existing_cuboid.max_x <= new_cuboid.max_x
        && existing_cuboid.min_y >= new_cuboid.min_y && existing_cuboid.max_y <= new_cuboid.max_y
        && existing_cuboid.min_z >= new_cuboid.min_z && existing_cuboid.max_z <= new_cuboid.max_z {
            return Intersect::FullyInside;
    }

    let mut parts: Vec<Cuboid> = vec![
        existing_cuboid.clone()
    ];

    for (axe, value) in [
        (Axe::X, new_cuboid.min_x),
        (Axe::X, new_cuboid.max_x),
        (Axe::Y, new_cuboid.min_y),
        (Axe::Y, new_cuboid.max_y),
        (Axe::Z, new_cuboid.min_z),
        (Axe::Z, new_cuboid.max_z),
    ] {
        let mut new_parts: Vec<Cuboid> = vec![];

        for part in parts {
            let (a, b, c) = cut_cuboid(part.clone(), axe, value);
    ...
    Intersect::SomePartsOutside(parts.into_iter().filter(|part| {
        let intersection = intersecting(new_cuboid, part);
        assert!(intersection == Intersect::FullyOutside || intersection == Intersect::FullyInside);

        intersection == Intersect::FullyOutside
    }).collect())
}

fn cut_cuboid(cuboid: Cuboid, axe: Axe, value: i32) -> (Cuboid, Option<Cuboid>, Option<Cuboid>)
{
    let (min, max) = match axe {
        Axe::X => (cuboid.min_x, cuboid.max_x),
        Axe::Y => (cuboid.min_y, cuboid.max_y),
        Axe::Z => (cuboid.min_z, cuboid.max_z),
    };

    let (a, b, c) = if min == max || value < min || value > max {
        ((min, max), None, None)
    } else if value == min {
        ((value, value), Some((value + 1, max)), None)
    } else if value > min && value < max  {
        ((value + 1, max), Some((value, value)), Some((min, value - 1)))
    } else if value == max {
        ((min, value - 1), Some((value, value)), None)
    } else {
        panic!();
    };
    ...
    match axe {
        Axe::X => (
            Cuboid { min_x: a.0, max_x: a.1, ..cuboid },
            b.map(|(min, max)| Cuboid { min_x: min, max_x: max, ..cuboid }),
            c.map(|(min, max)| Cuboid { min_x: min, max_x: max, ..cuboid }),
        ),
        Axe::Y => (
            Cuboid { min_y: a.0, max_y: a.1, ..cuboid },
            b.map(|(min, max)| Cuboid { min_y: min, max_y: max, ..cuboid }),
            c.map(|(min, max)| Cuboid { min_y: min, max_y: max, ..cuboid }),
        ),
        Axe::Z => (
            Cuboid { min_z: a.0, max_z: a.1, ..cuboid },
            b.map(|(min, max)| Cuboid { min_z: min, max_z: max, ..cuboid }),
            c.map(|(min, max)| Cuboid { min_z: min, max_z: max, ..cuboid }),
        ),
    }

    // match axe {
    //     Axe::X => {
    //         if value < cuboid.min_x || value > cuboid.max_x {
    //             (cuboid, None)
    //         } else {
    //             (
    //                 Cuboid { max_x: value - 1, ..cuboid },
    //                 Some(Cuboid { min_x: value, max_x: value, ..cuboid })
    //                     Cuboid { min_x: value + 1, ..cuboid },
    //                 ))
    //             )
    //         }
    //     }
    //     Axe::Y => {
    //         if value < cuboid.min_y || value > cuboid.max_y {
    //             (cuboid, None)
    //         } else {
    //             (
    //                 Cuboid { max_y: value - 1, ..cuboid },
    //                 Some((
    //                     Cuboid { min_y: value, max_y: value, ..cuboid },
    //                     Cuboid { min_y: value + 1, ..cuboid },
    //                 ))
    //             )
    //         }
    //     }
    //     Axe::Z => {
    //         if value < cuboid.min_z || value > cuboid.max_z {
    //             (cuboid, None)
    //         } else {
    //             (
    //                 Cuboid { max_z: value - 1, ..cuboid },
    //                 Some((
    //                     Cuboid { min_z: value, max_z: value, ..cuboid },
    //                     Cuboid { min_z: value + 1, ..cuboid },
    //                 ))
    //             )
    //         }
    //     }
...
fn parse_range(expected_axe: &'static str, string: &'static str) -> (i32, i32)
{
    let (axe, range_string) = string.split_once('=').unwrap();
    assert_eq!(expected_axe, axe);
...
{
    {
        let operation = Cuboid { status: Status::On, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
        let (a, b, c) = cut_cuboid(operation, Axe::X, 2);

        assert_eq!(Cuboid { status: Status::On, min_x: 3, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }, a);
        assert_eq!(Some(Cuboid { status: Status::On, min_x: 2, max_x: 2, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }), b);
        assert_eq!(Some(Cuboid { status: Status::On, min_x: 0, max_x: 1, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }), c);
    }

    {
        let operation = Cuboid { status: Status::On, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
        let (a, b, c) = cut_cuboid(operation, Axe::X, 3);

        assert_eq!(Cuboid { status: Status::On, min_x: 0, max_x: 2, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }, a);
        assert_eq!(Some(Cuboid { status: Status::On, min_x: 3, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }), b);
        assert_eq!(None, c);
    }

    {
        let operation = Cuboid { status: Status::Off, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
        let existing_cuboid = Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 2, max_y: 4, min_z: 2, max_z: 4};
    
        assert_eq!(Intersect::FullyOutside, intersecting(&operation, &existing_cuboid));
    }

    {
        let operation = Cuboid { status: Status::Off, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
        let existing_cuboid = Cuboid { status: Status::On, min_x: 2, max_x: 3, min_y: 2, max_y: 3, min_z: 2, max_z: 3};
    
        assert_eq!(Intersect::FullyInside, intersecting(&operation, &existing_cuboid));
    }

    // {
    //     let operation = Cuboid { status: Status::Off, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
    //     let existing_cuboid = Cuboid { status: Status::On, min_x: 3, max_x: 4, min_y: 2, max_y: 4, min_z: 2, max_z: 4};
    
    //     assert_eq!(Intersect::SomePartsOutside(vec![
    //         Cuboid { status: Status::On, min_x: 3, max_x: 3, min_y: 2, max_y: 3, min_z: 4, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 3, min_y: 4, max_y: 4, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 3, min_y: 4, max_y: 4, min_z: 4, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 2, max_y: 3, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 2, max_y: 3, min_z: 4, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 4, max_y: 4, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 4, max_y: 4, min_z: 4, max_z: 4 },
    //     ]), intersecting(&operation, &existing_cuboid));
    // }

    // {
    //     let operation = Cuboid { status: Status::Off, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
    //     let existing_cuboid = Cuboid { status: Status::On, min_x: 2, max_x: 4, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
    
    //     assert_eq!(Intersect::SomePartsOutside(vec![
    //         Cuboid { status: Status::On, min_x: 4, max_x: 4, min_y: 0, max_y: 3, min_z: 0, max_z: 3 }
    //     ]), intersecting(&operation, &existing_cuboid));
    // }

    // {
    //     let operation = Cuboid { status: Status::Off, min_x: 0, max_x: 3, min_y: 0, max_y: 3, min_z: 0, max_z: 3};
    //     let existing_cuboid = Cuboid { status: Status::On, min_x: 2, max_x: 4, min_y: 2, max_y: 4, min_z: 2, max_z: 4};
    
    //     assert_eq!(Intersect::SomePartsOutside(vec![
    //         Cuboid { status: Status::On, min_x: 2, max_x: 3, min_y: 2, max_y: 3, min_z: 3, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 2, max_x: 3, min_y: 3, max_y: 4, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 2, max_x: 3, min_y: 3, max_y: 4, min_z: 3, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 4, min_y: 2, max_y: 3, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 4, min_y: 2, max_y: 3, min_z: 3, max_z: 4 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 4, min_y: 3, max_y: 4, min_z: 2, max_z: 3 },
    //         Cuboid { status: Status::On, min_x: 3, max_x: 4, min_y: 3, max_y: 4, min_z: 3, max_z: 4 },
    //     ]), intersecting(&operation, &existing_cuboid));
    // }

    assert_eq!(39, number_of_ons("
    ...
    assert_eq!(590784, number_of_ons(test_input()));
}
